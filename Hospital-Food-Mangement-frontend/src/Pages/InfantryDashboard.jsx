    ...
    <div className="max-w-7xl mx-auto px-4">
      <div className="flex justify-between h-16">
        <div className="flex items-center">
          ...
          <span className="ml-2 text-lg font-semibold">Infantry Dashboard</span>
        </div>
        <button
          onClick={onLogout}
...
const Aside = ({ staffDetails }) => (
  <aside className="fixed left-0 top-16 h-full w-64 bg-white border-r border-gray-200 p-4">
    ...
    <div className="space-y-4">
      ...
      <div className="border-b pb-4">
        <h3 className="text-sm font-medium text-gray-500 mb-2">Shift Info</h3>
      ...
      <div>
        ...
        <div className="bg-orange-50 p-3 rounded">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="font-semibold">{staffDetails.mealsPrepped}</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2.5">
            <div 
              className="bg-orange-600 h-2.5 rounded-full" 
              style={{ width: `${(staffDetails.mealsPrepped / staffDetails.totalMealsToday) * 100}%` }}
            ></div>
          </div>
          <p className="text-xs text-gray-500 mt-1">
            {staffDetails.mealsPrepped} of {staffDetails.totalMealsToday} meals
          </p>
        </div>
      </div>
    </div>
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <div className="flex items-center mb-2">
                      <h2 className="text-lg font-semibold">{task.patientName}</h2>
                      <span className="ml-2 px-2 py-1 bg-orange-100 text-orange-800 text-xs rounded-full">
                        {task.mealType}
                      </span>
                    </div>
                    <p className="text-gray-600">
                      <span className="font-medium">Meal ID:</span> {task.mealId}
                    </p>
                    <p className="text-gray-600">
                      <span className="font-medium">Dietary Requirements:</span> {task.dietaryRequirements}
                    </p>
                    <p className="text-gray-600">
                      <span className="font-medium">Allergies:</span> {task.allergies.join(', ')}
                    </p>
                    <p className="text-gray-600">
                      <span className="font-medium">Prep Time:</span> {task.prepTime}
                    </p>
                  </div>
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex flex-col items-end">
                      <select
                        ...
                        onChange={(e) => handleAssignDelivery(task.id, e.target.value)}
                        defaultValue=""
                      >
                        <option value="" disabled>Select Delivery Staff</option>
                        {deliveryStaff.map((staff) => (
                          <option key={staff.id} value={staff.id}>
                            {staff.name} ({staff.activeDeliveries} active)
                          </option>
                        ))}
                      </select>
                    </div>
